...
public class PersonServiceImpl implements PersonService, CommandLineRunner {
	...
	public PersonDto updatePersonName(Integer id, String name) {
		Person person = personRepository.findById(id).orElseThrow(PersonNotFoundException::new);
		person.setName(name);
		savePersonDto(modelMapper.map(person, PersonDto.class));
		return modelMapper.map(person, PersonDto.class);
	}
	...
	public Iterable<PersonDto> findPersonsByCity(String city) {
		return personRepository.findByAddressCityCase(city).map(p -> modelMapper.map(p, PersonDto.class))
	...
	public Iterable<PersonDto> findPersonsByName(String name) {
		return personRepository.findByNameIgnorCase(name).map(p -> modelMapper.map(p, PersonDto.class))
				.collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public Iterable<EmployeeDto> findEmployeesBySalary(double minSalary, double maxSalary) {
	    return personRepository.findEmployeesBySalaryBetween(minSalary, maxSalary).stream()
	...
	private void savePersonDto(PersonDto personDto) {
